...
def _row_to_dict(row):
    try:
        ...
        try:
            ...
            return row  # fallback; may still be subscriptable
...
def tnp_view(client, models):
                ...
                if jd_text:
                    ...
                    if jd_data and isinstance(jd_data, dict) and jd_data.get("roles"):
                        ...
                        for role in jd_data["roles"]:
                            ...
                            # Store role JSON (dict) for later scoring
    ...
    for job in _rows_to_dicts(db.get_tnp_jobs(st.session_state.user_id)):
            ...
            if st.form_submit_button("Apply") and resume_file and selected_job_str:
                with st.spinner("Analyzing your resume and submitting application..."):
                    ...
                    if resume_text and job_details:
                        ...
                        # Load structured JD role stored with the job
                        jd_role = None
                        try:
                            # Adjust the key names based on your DB schema
...
if st.session_state.logged_in:
    # Initialize Gemini client separately to avoid masking downstream errors
    try:
