...
DEFAULT_GENAI_MODEL_EMBED = "text-embedding-004"  # OK in SDK; consider gemini-embedding-001 if migrating later
...
def _make_genai_client(api_key: str | None) -> genai.Client:
    ...
    - If api_key is provided, uses it explicitly.
    - Else relies on env: GOOGLE_API_KEY or GEMINI_API_KEY (GOOGLE_API_KEY has precedence).
    """
    if api_key and api_key.strip():
        return genai.Client(api_key=api_key.strip())
    # If neither provided nor env set, Client() will still try env; raise if missing.
    ...
    return genai.Client()  # auto-detects env keys

# -------------------- Gemini wrappers (Client-based) --------------------
def _genai_generate_text(client: genai.Client, model: str, prompt: str) -> str:
    """
    Calls Gemini to generate text using the Client API; returns plain text.
    """
    resp = client.models.generate_content(model=model, contents=prompt)
    # The SDK exposes resp.text as the aggregated text
    return getattr(resp, "text", None) or ""

def _genai_embed_text(client: genai.Client, model: str, text: str) -> list[float]:
    """
    Calls Gemini embeddings model via Client API; returns list[float].
    """
    eresp = client.models.embed_content(model=model, contents=text)
    # python-genai returns eresp.embeddings; each item has values
    # For single input, embeddings[0].values should be the vector
    # Some SDK versions use eresp.embeddings[0].values; fall back if necessary.
    if hasattr(eresp, "embeddings") and eresp.embeddings:
        ...
        if vec is not None:
            return list(vec)
    # Older docs show dict-like; try mapping
    try:
            ...
            with st.spinner("Processing JD..."):
                ...
                if jd_text:
                    ...
                    if jd_data and jd_data.get("job_title"):
                        ...
                        st.error("Could not extract required details from the JD.")
                else:
            ...
            if st.form_submit_button("Apply") and resume_file and selected_job_str:
                with st.spinner("Analyzing your resume and submitting application..."):
                    ...
                    if resume_text and job_details:
                        ...
                        score = calculate_semantic_similarity(client, models['embed'], job_details['jd_text'], resume_text)
                        if resume_data:
